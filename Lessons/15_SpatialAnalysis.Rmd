...
#Plot the data using longitude and latitude as X and Y; longitude is flatatude (X axis); latitude is Y axis
ggplot(EPAair_PM_avg, aes(x=SITE_LONGITUDE, y=SITE_LATITUDE)) + 
  geom_point(aes(color=meanPM), size=4) + coord_equal() #coord_equal() scales the axes better
```
...
myMap  <- leaflet(data=EPAair_PM_avg) %>% 
  addTiles() %>% #need addTiles
  addCircleMarkers(~SITE_LONGITUDE,~SITE_LATITUDE,
                   radius=(~meanPM*2),
                   stroke = FALSE, #stroke is the outline
                   fillOpacity = 0.3,
                   popup = ~as.character(`Site.Name`)) #code that specifies what pops up when you click on the circle 
...
Durham_buffer <- EPAair_PM_avg_sf %>% 
  ...
  st_buffer(0.1) #buffering points by 0.1 degrees
...
EPAair_PM_avg_sf_UTM <- st_transform(EPAair_PM_avg_sf, crs=26917) #takes initial data frame and converts it to UTM
...
Distances_to_DASite <- EPAair_PM_avg_sf_UTM %>% 
  ...
  data.frame() %>%  t                         #Transpose the result; converts data to data frame and transposes it 
...
counties_sf<- st_read('./Data/Spatial/cb_2017_us_county_20m.shp') %>% 
  filter(STATEFP == 37) #Filter for just NC Counties; st_read reads shape_file provided

#CRS (Is this the same as the EPA Dataset?); CRS - coordinate reference system
st_crs(counties_sf)
...
#Reveal the extent of this dataset via the st_bbox() function; bbox short for bounding box
st_bbox(counties_sf)
...
#Check the CRS
st_crs(huc8_sf) #no EPSG code given because one is not always given
...
plot(huc8_sf['SUB'],
     graticule = st_crs(4326),  #graticule is a grid you can put on the back of your plot; need to give graticule a coordinate references system
     axes = TRUE)
...
state_sf <- counties_sf %>% 
  group_by(STATEFP) %>% 
  summarize(ALAND = sum(ALAND)) #have counties become one feature
...
#Now plot with leaflet: no errors as all layers are in Leaflet's native CRS (WGS84); can add more than one layer to the map and can change colors; leaflet can only plot in wfs84
leaflet() %>% addTiles() %>% 
  addPolygons(data=counties_WGS84,weight=1,color='red') %>%  #add county layers
  addPolygons(data=huc8s_WGS84,weight=1) #add huc layers
```
...
#order matters! Put basemap on first, then everything that is on top after it
...
leaflet() %>% #create leaflet object
  addProviderTiles(providers$Esri.WorldShadedRelief) %>%  #see link above to see what basemaps are available
  addPolygons(data = counties_WGS84, color = "orange", weight = 1, smoothFactor = 0.5,    #what you want to display as polygons and their settings
              opacity = 1.0, fillOpacity = 0.5,
              fillColor = ~colorQuantile("YlGnBu", ALAND)(ALAND)) %>% 
  addPolygons(data = huc2_WGS84, color=NA, weight = 2) %>%  #adding additional polygons
  addMarkers(data=EPAair_wgs84,popup = ~as.character(`Site.Name`)) #adding markers; can play around with the shape of the markers
...
latticeview(m1, m2)  #puts maps into a grid to show them side by side
sync(m1,m2)  #syncs maps so that zoom in and zoom out happens on both maps at the same time
```
...
huc0302 <- huc2_utm %>% 
  filter(SUB == '0302') #need to put 0302 in quotes because they are factors and are not numeric in R

huc0302_counties <- st_intersection(counties_sf_utm, huc0302)  #did this in two lines becuase piping and interections are tricky and sometimes don't work well
...
demog_df <- read.csv('./Data/Spatial/NC_Demography.csv') %>% 
  mutate(FIPS = as.factor(FIPS))                                 #Convert the FIPS variable to a factor even though they are numbers; converting to factors keeps leading '0' stays in the column

#Join the two datasets using "GEOID" from the left dataset and "FIPS" from the right; use left-join which specifies a left table (which is our counties spatial features) as our x value; and join with the y table by the two columns you want to join; in counties, the column is GEOID and in the demography dataset it's FIPS
...
#use 'st as sf' to say that it's a spatial data frame if doing a right-table join; left-join will automatically keep the spatial information
```
...
triCo_centroids <-  st_centroid(triCo)     #st_centroid which takes four geometries and overwrite with values of the centroid
myMap <- myMap + geom_sf(data = triCo_centroids, color = 'blue')
myMap

#Buffer the centroids outward 2km and add them to our
triCo_centroids_2km <- st_buffer(triCo_centroids, 2000) #can buffer points
myMap <- myMap + geom_sf(data = triCo_centroids_2km, color = 'orange', fill=NA)
myMap

#Buffer the counties inward 2km
triCo_in2km <- st_buffer(triCo, -2000) #shrink counties by 2000 meteres
...
triCo_centroids_chull <- triCo_centroids %>% 
  st_union() %>%   #takes centroids in 4 rows and comvines into one row
  st_convex_hull()
myMap <- myMap + geom_sf(data = triCo_centroids_chull, color = 'red', fill=NA)
myMap

#Combine the centroids into one feature and draw voronoi polygons; useful wehn looking at interpolation; could be useful for EPA sites to figure out their boundaries
...
#User coordinates for the point
userLat = 36.0045442
userLng = -78.9426381
...
theSite_sfc <- st_sfc(theSite_sfp, crs = 4326) #creating a spatial feature column; cooridnates + spatial reference
...
resultMask <- st_intersects(counties_sf_utm,     #st_intersects is different from st_intersection
                            theSite_sfc_transformed,
                            sparse = FALSE) #The `sparse` option returns a Boolean mask; gives mainly FALSE answers, so next line of code asks for it to return TRUE answers
View(resultMask)
...
#take one coordinate and buffer it, instead of interescting it; then intersect the circle from the buffer with counties and then it will give results that are more accurate; in this case will give buffer 30km from the points
