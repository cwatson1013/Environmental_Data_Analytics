...
format(today, format = "%A/%b/%m")
format(today, format = "%b/%Y/%d")
format(today, format = "%a/%Y/%b")
...
```{r}
#all dates prior to 1969 are stored as 2000 year, but they should be 1900

#change the format of the date to two digit year, month, and day as a number; creates a character string of dates
USGS.flow.data$datetime <- format(USGS.flow.data$datetime, "%y%m%d")

#function where if date is greater than 181231, put a 19, if not, put a 20; paste0 will take two statements inside the function and put them together; writing the recepie
create.early.dates <- (function(d) {
       paste0(ifelse(d > 181231,"19","20"),d)
       })
#in the datetime column, use the function to update the date format with the function written above by adding a 19 or a 20
USGS.flow.data$datetime <- create.early.dates(USGS.flow.data$datetime)

#update class/format so that dates in columns are seen as date and not a factor
...
> Answer: Remove NAs when that represents data that is missing, or is blank. Leave NAs in data when that means something (i.e. water quality measurements may be undetecable and not a blank value). When you have data like we have here, with a bunch of blank cells and means that data wasn't collected which means you could eliminate the NAs so we are only analyzing years when there was complete monitoring of variables.  

```{r}
#omitting NAs from the data set 
USGS.flow.data.complete <- na.omit(USGS.flow.data) #will delete entire row of data, even if some cells are filled in
dim(USGS.flow.data) #comparing dimensions of USGS Flow Data and Flow Data Complete
dim(USGS.flow.data.complete) #this data frame only has cells filled in those columns
...
```{r, fig.height = 3, fig.width = 4}
#creating a histogram of the discharge.mean 
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean))

#historgram of discharge.mean data with binwidth = 10; divided mean discharge values into creating a bin every 10 units in units of whatever the column unit is
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean), binwidth = 10)

#histogram of dishcarge.mean data with number of bins = 20; will divide data up accordingly
ggplot(USGS.flow.data.complete) +
  geom_histogram(aes(x = discharge.mean), bins = 20)

#histogram of discharge.mean data with binwidth = 10, and a continuous x-axis scale from 0 to 500; zoomed in to all mean discharge values under 500 to see what is going on while not looking at the tails; this is fine to do in data exploration phase
...
```{r, fig.height = 3, fig.width = 4}
#great to visualize different distributions of data; create 3 frequency polygons: 1 for mean gage height, 1 for min gage heigh, 1 for max gage height; sepcify colors, bin numbers, and scale limit
ggplot(USGS.flow.data.complete) +
...
#taking measurements in each column and dividing up based on 
#take the mean and splitting up based on what category it fell into - either approval or pending; same as above, but just using one line and letting ggplot choose the color
ggplot(USGS.flow.data.complete) +
...
```{r, fig.height = 3, fig.width = 4}
#creating a boxplot for gage height mean approval vs gage height mean
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean.approval, y = gage.height.mean))

#boxplot of mean gage height vs discharge mean; divide gage height mean into groups of 1; higher the gage height, the heigher the discharge value; useful if you want to divide up two continuous variables into box and whisker plots; useful when you have a nonlinear releationship
ggplot(USGS.flow.data.complete) +
  geom_boxplot(aes(x = gage.height.mean, y = discharge.mean, group = cut_width(gage.height.mean, 1)))

#violin plots with quantiles specified; greater amount of samples, the wider the plot
ggplot(USGS.flow.data.complete) +
