...
```{r}
irradiance.regression <- lm(PeterPaul.chem.nutrients$irradianceWater ~ PeterPaul.chem.nutrients$depth) #explanatory and response on right sides of ~
# another way to format the lm function
irradiance.regression <- lm(data = PeterPaul.chem.nutrients, irradianceWater ~ depth)
summary(irradiance.regression) #only had to note dataframe once

#adjusted R squared value will penalize you for using multiple varibles

# Correlation
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth) #testing correlation between two variables
```
Question: How would you report the results of this test (overall findings and report of statistical output)?

> ANSWER: There is a negatvie correlation betwen depth and irradiance water (df = 15449, adjusted R squared = 0.3091, p-value < 0.0001, cor = -0.56).
> Irradiance is negatively correlted with depth (linear regression; R2 = 0.31, p < 0.001, df = 15449) use p-value for whole sample if using multiple variable (p-value at the bottom). Can put equation on figure to show what it is. 
...
plot(irradiance.regression) #one point has a really high residual
```
...
irradiancebydepth <- 
  ...
  ylim(0, 2000) + #limiting the y axis to take out the outlier
  geom_point() 
...
irradiance.regression2 <- lm(data = PeterPaul.chem.nutrients, log(irradianceWater) ~ depth) #log transform data; response variable is log transformed
...
irradiancebydepth2 <- 
  ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
  geom_smooth(method = "lm") + #don't use blue line (ggplot default)
  scale_y_log10() + #easy way to plot what you want; puts breaks on a log scale
  geom_point() 
print(irradiancebydepth2) 

# SE can also be removed; remove confidence interval by saying SE = FALSE
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point() +
    scale_y_log10() +
...
irradiancebydepth2 <- 
    ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = irradianceWater)) +
    geom_point(alpha = 0.2) + #0.5 makes them 50% transparent; 0.2 makes them 80% transparent
    scale_y_log10() +
    labs(x = "Depth (m)", y = "Water Irradiance (units)") +
    scale_x_continuous(breaks = c(0, 2, 4, 6, 8, 10, 12)) + #changing the x-axis scale
    geom_smooth(method = 'lm', se = FALSE, color = "purple") #coud change the color of the points; transparency of points +
    #geom_text(expression("")) # could let you put the equation on the graph
print(irradiancebydepth2)
...
cor.test(PeterPaul.chem.nutrients$irradianceWater, PeterPaul.chem.nutrients$depth, 
         method = "spearman", exact = FALSE) #use spearman's method to conduct nonparametric variant to the linear regression; nonparametric equivalent 
```
...
PeterPaulnutrients <- 
  ...
  select(tn_ug:po4) %>%
  na.omit() #omitted the NA's; need to have a full data set with NO NAs - otherwise it won't run
PeterPaulCorr <- cor(PeterPaulnutrients) #find correlation between variables in the dataframe
corrplot(PeterPaulCorr, method = "ellipse")
corrplot.mixed(PeterPaulCorr, upper = "ellipse") #correlation plot with correlation amounts and ellipses 
```
...
TPAIC <- lm(data = PeterPaul.naomit, tp_ug ~ depth + dissolvedOxygen + 
              temperature_C + tn_ug + po4) #create linear model 
step(TPAIC) #function step gives us AIC from top of model; increase = TRUE has step start from bottom and work up; lower AIC is better; and runs the linear model with only variables that have low AIC scores included
TPmodel <- lm(data = PeterPaul.naomit, tp_ug ~ temperature_C + tn_ug)
summary(TPmodel) #explained almost 54% of total phosphorous; each unit of nitrogen, we increase phosphorous, and each unit of temperature, phosphorous decreases by -0.468
...
```{r, fig.height = 3, fig.width = 4}
# main effects
TNancova.main <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename + depth)  #categorical variable = lakename
summary(TNancova.main) #main effects ANCOVA because we used '+' in the code; effect of lakename and depth

# interaction effects; interaction between total nitrogen and depth
TNancova.interaction <- lm(data = PeterPaul.chem.nutrients, tn_ug ~ lakename * depth) 
summary(TNancova.interaction)

#depth = depth in Paul lake because that refers to Paul Lake which was the first variable in the intercept; gaining more conceptual information about our system

TNplot <- ggplot(PeterPaul.chem.nutrients, aes(x = depth, y = tn_ug, color = lakename)) +  #color based on lakename; should have two lines of best fit because these are in our aesthetics
  geom_point() + 
  geom_smooth(method = "lm", se = FALSE) +
  xlim(0, 10)
print(TNplot)

#might log transform peter lake information because of the outliers/large points
