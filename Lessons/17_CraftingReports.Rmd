...
1. **YAML Header** (Yet Another Markdown Lesson) surrounded by --- on top and bottom
...
2. **R Code Chunks** surrounded by ``` on top and bottom
    + Create using `Cmd/Ctrl` + `Alt/Option` + `I`
    + Can be named {r name} to facilitate navigation and autoreferencing
    + Chunk options allow for flexibility when the code runs and when the document is knitted; determine what shows in our document, what runs, what doesn't run
...
- Simple syntax and autoreferencing (will need to autoreference figures and tables in R for final project)
...
Dataset Name | Detail about dataset
-------------|---------------------
NTLER        | Nutrients in lakes
Ecotox       | Neonicotinoids
EPA Air      | Air quality
USGS         | Stream flow data
NTLER        | Carbon in lakes
NTLER        | Lake chemistry and physical data
...
getwd()
require(tidyverse) #requires doesn't show warnings; library does show warnings
require(knitr)
require(viridis)
...
nutrientsprocessed <- nutrients %>%
  select(-lakeid, -depth_id, -comments) %>%  #for filter use &, for select use commas to indidcate and
  filter(depth == 0)
```
...
nutrientssum_tn <- nutrientsprocessed %>%
  group_by(lakename) %>%
...
nutrientssum_tp <- nutrientsprocessed %>%
  group_by(lakename) %>%
...
```{r, echo=FALSE, eval=TRUE}
kable(nutrientssum_tn, caption = "Total N in Lakes")
kable(nutrientssum_tp, caption = "Total P in Lakes")
...
```{r, echo = FALSE, fig.cap="Total Nitrogen in Lakes", warning=FALSE}

#plot of Total N over time 
totalN <- ggplot(nutrientsprocessed, aes(x = sampledate, y = tn_ug, color = lakename)) +
  geom_point() +
  labs(x = "Date", y = "Total Nitrogen (micrograms)", color = "Lake Name") +
  scale_color_brewer(palette = "YlGnBU")
print(totalN)
```

```{r, echo=FALSE, fig.cap="Total Phosphorous in Lakes", warning=FALSE}
#plot of Total P over time
totalP <- ggplot(nutrientsprocessed, aes(x = sampledate, y = tp_ug, color = lakename)) +
  geom_point() +
  labs(x = "Date", y = "Total Phosphorous (micrograms)", color = "Lake Name") +
  scale_color_brewer(palette = "YlGnBU")
print(totalP)
