...
What is the output that results? The file path where the working directory is and where files are stored. 
...
#setwd("/Users/katerisalk/Documents/Duke/Courses/Environmental_Data_Analytics") # may not work well because this is a path for Kateri's computer, so won't work on mine
```
...
USGS.flow.data <- read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv") # ./ tells R that it needs to go to one file below the working directory file

# What happens if we don't assign a name to our imported dataset?
#read.csv("./Data/Raw/USGS_Site02085000_Flow_Raw.csv") # it doesn't save it as a data/dataframe and will put it into the console, but you won't be able to "save" it or see it again

# Another option is to choose with your browser
 #read.csv(file.choose()) # don't know what you are looking for; not reproducible; if you want to use this, you'll want to leave a comment saying what you imported
...
> ANSWER: This file is located in the Raw file in the Data file of the Env_Data_Analytics file. The placement and information found in this file relates to best practices because it is descriptive.
...
str(USGS.flow.data) # means structure
dim(USGS.flow.data) # means dimensions; will give you the dimensions of the data frame (number of rows and columns)
length(USGS.flow.data) # length gives number of columns

head(USGS.flow.data) #will give the first couple rows of the 15 columns
head(USGS.flow.data, 10) #will give you the 10 first rows of data frame
tail(USGS.flow.data, 5) #will give the tail end of the data
USGS.flow.data[30000:30005, c(3, 8, 14)] #can matrix your data to see rows 30000 to 30005 and say you want column 3, 8, and 14
