...
The coefficients of a hierarchical model are divided into two categories: **fixed effects** and **random effects.** A **fixed effect** is a factor whose levels are experimentally determined or whose interest lies in the effects of each level (e.g., covariates, treatments, interactions). A **random effect** is a factor whose levels are sampled from a larger population, or whose interest lies in the variation among them rather than the specific effect of each level. In choosing whether you are dealing with a fixed or a random effect, consider the following questions: 
  ...
  + Have you included all possible levels in the study? --> random effect
  
  + Do you have interest in the variance among levels? --> random effect if you want to know the differences in the variance 
...
PeterPaul.summertemp <- 
  ...
  mutate(Week = week(sampledate)) %>%  #week function from lubridate; will populate the column with the week that the sampledate is in
...
```{r}
# Determine autocorrelation in residuals; temperatures on a single day are dependent on temperatures during different days; lme is associated with nlme
TempTest.auto <- lme(data = PeterPaul.summertemp,
                     temperature_C ~ sampledate * lakename, #temp is response variable, predicted based on sampledate and lakename
                     random = ~1|Week) #specifying random effect in this line; ~1 means that each week has it's own intercept in the model 
TempTest.auto #interested in knowing the variablity among the weeks; so we are given an std of tempearature accross the season for each week

ACF(TempTest.auto) #quantify the autocorrelation associated with the model; one that is associated with 0 is always going to be 1; we take the second value to describe the degree of autocorrelation; here we have an autocorrelation value of 42, so 42% of our variables are associated with autocorrelation over time.
...
TempTest.mixed <- lme(data = PeterPaul.summertemp,
                     ...
                     #sampledate is duplicated in some cases, so need to split up by lake so that it is taking sample dates associated with a lake at one time
                     correlation = corAR1(form = ~ sampledate/lakename|Week, value = 0.423), #spatial autocorrelation would be put where we have |Week
                     #define method as restricted maximum likelihood
                     method = "REML") 
summary(TempTest.mixed) #intercept=20.5, which is the very first day we sampeld at Paul lake; sampledate effect shows that we are going to increase our temperature by 0.0000009 each day; 

# Compare the random effects model with the fixed effects model; using gls (generalized linear model), with no defined random effect or autocorrelation structure
TempTest.fixed <- gls(data = PeterPaul.summertemp,
                      temperature_C ~ sampledate * lakename, 
                      method = "REML")
summary(TempTest.fixed) #getting about the same conclusion from the fixed effects model as with the random effects model

#what was the point of doing a random effect and fixed effect model? Dangers of only looking at p-values and coefficients in the model 

anova(TempTest.mixed, TempTest.fixed) #this compares the models; p-value is < 0.0001 so model fit is significantly different; AIC shows fixed model accounts for less variability in the model; have more error accounted for in the mixed effect of week 

# The lower the AIC, the better.
# The p-value tells us whether those models have a significantly different fit

# Post-hoc test - what you would do if you had multiple variables that had varying differences
...
ggplot(PeterPaul.summertemp) +
  geom_point(aes(x = sampledate, y = temperature_C, color = lakename)) +
  labs(x = "Date", y = "Temperature (C)", color = "Lake Name") +
  scale_color_manual(values = c("#7fcdbb", "#253494")) +
  geom_text(x = as.Date("2010-01-01"), y = 28, label ="y = 0.0001x + 20.47") + 
